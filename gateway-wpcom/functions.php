...
function custom_excerpt() {
    echo '<p style="margin-right:28px;font-family:Lexend;font-size:15px; color:#FFFFFF99;">' . get_the_excerpt() . '</p>';
}
function custom_posted_on(){
	echo '<p style="font-family:Lexend;font-size:11px;color:##FFFFFF5C;">'.get_the_date().'</p>';
}
function custom_content(){
	$content = apply_filters( 'the_content', get_the_content() );
		echo '<p style="font-size:15px;color:#FFFFFFDE;font-family:Lexend;">'.$content.'</p>';
}
function custom_search_form() {
    $form = '<div class="search-form-wrap">';
    $form .= get_search_form(false);
    $form .= '</div>';
    return $form;
}
function remove_search_button($form) {
    $form = str_replace('<input type="submit"','<input type="submit" style="display:none;"',$form);
    return $form;
}
add_filter('get_search_form', 'remove_search_button');
